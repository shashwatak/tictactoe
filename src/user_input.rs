...
const MAX_ROW_CHAR: char = ('1' as u8 + NUM_ROWS as u8) as char;

const MIN_COLUMN_CHAR: char = 'a';
const MAX_COLUMN_CHAR: char = ('a' as u8 + NUM_COLS as u8) as char;
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        match input.chars().collect::<Vec<char>>()[..] {
            ...
            [row_char, column_char] => {
                if row_char < MIN_ROW_CHAR || row_char > MAX_ROW_CHAR || column_char < MIN_COLUMN_CHAR || column_char > MAX_COLUMN_CHAR {
                    return Err(Self::Err::OutOfBounds);
                }
